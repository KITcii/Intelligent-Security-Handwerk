...
public class TermRepositoryCustomImpl implements TermRepositoryCustom {
    ...
    @Override
    public <T> Page<T> findAllBySpecification(Specification<Term> spec, Pageable pageable, Class<T> dtoType) {
        ...
        // Project to DTO
        query.select(entityManager.getCriteriaBuilder().construct(dtoType, root.get("id"), root.get("keyword")));
        ...
        Root<Term> countRoot = countQuery.from(Term.class);
